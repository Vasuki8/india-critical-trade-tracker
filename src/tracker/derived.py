...
def derive_unit_values(
    ...
    """Match USD and quantity reports and derive transparent implied unit values.

    TradeStat monetary observations are stored in USD million. Quantity observations
    retain the source unit reported by TradeStat. We only calculate a unit value when
    the same trade direction and HS8 code exist in both documents and the quantity is
    positive with a non-empty source unit.
    """
    ...
    for trade_type, hs_code in keys:
        ...
        quantity = _total_value(quantity_report)
    ...
    return {
        ...
        "method": "USD million × 1,000,000 divided by TradeStat quantity in its reported source unit",
        "aggregate": aggregate,
